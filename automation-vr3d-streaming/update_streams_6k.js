...
// --- CONSTANTS ---
const DRY_RUN = false; // Set to true initially for safety
const LIMIT_UPDATES = 1000;
const API_BASE_URL = 'devgateway.techxrdev.in';
const API_PATH_PREFIX = '/api/content/cms/episodes/';
// Using same token as previous script
...
async function main() {
    try {
        ...
        // Extract items from apiResponse
        let apiCards = [];
        ...
        const TARGET_PATH_SUBSTRING = '/DurlabhDarshan6K/Encrypted/Hindi_100B/';
        ...
        for (const key in videoThumbnails) {
            ...
            if (!adaptiveUrl || !adaptiveUrl.includes(TARGET_PATH_SUBSTRING)) {
                continue;
            }

            // Extract the base folder from the Adaptive URL
            // URL format: .../DurlabhDarshan6K/Encrypted/Hindi_100B/<FOLDER_NAME>/...
            const parts = adaptiveUrl.split(TARGET_PATH_SUBSTRING);
            if (parts.length < 2) continue;

            // Assuming structure end with /master.m3u8 or /master_6K.m3u8
            // We want the folder name immediately following the TARGET_PATH_SUBSTRING
            const subPath = parts[1];
            const folderName = subPath.split('/')[0];

            // e.g. "KedarnathDham_Hindi_Encrypted" if url is .../KedarnathDham_Hindi_Encrypted/master.m3u8
            ...
            // CONSTRUCT PAYLOAD
            const baseWebUrl = `https://s3.ap-south-1.amazonaws.com/co.techxr.system.backend.upload.dev/DurlabhDarshan6K/Encrypted/Hindi_100B/${folderName}`;
            const awsKey = vt.AdaptiveStreamVideoKeyAWSObjectId; // Using same key logic as before? 
            // Note: In the image, key is `encrypt_key.key` in the folder, but `awsKey` in payload usually refers to the path in DurlabhDarshanMobileApp bucket format.
            // We will trust the JSON `AdaptiveStreamVideoKeyAWSObjectId` as the source of truth for the payload's `awsKey` field.
            ...
            const newStreamingOptions = [
                ...
                {
                    "contentUrl": `${baseWebUrl}/output_6k/playlist.m3u8`,
                ...
                {
                    "contentUrl": `${baseWebUrl}/output_1k/playlist.m3u8`,
