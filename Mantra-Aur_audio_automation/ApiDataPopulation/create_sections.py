...
# INPUT_FILE = "Home.json"

# # Set DRY_RUN=True to preview requests without sending.
# DRY_RUN = False

# # Set LIMIT to a number (e.g. 5) to update only a few sections.
# # Set LIMIT=None to process all sections.
# LIMIT = None
...
# def create_section_request(conn, payload):
#     headers = {
...
#     endpoint = "/api/content/shubhdarshan/v1/admin/darshan-sections"
...
def main():
    try:
        ...
        if not isinstance(all_sections, list):
            print("Error: Input file must contain a JSON array of section payloads.")
            return

        # Slicing and Limiting
        sliced = all_sections[START_INDEX:]
        sections_to_process = sliced[:LIMIT] if LIMIT is not None else sliced

        print(f"Input Payloads: {len(all_sections)}")
        print(f"START_INDEX: {START_INDEX}")
        print(f"LIMIT: {'ALL' if LIMIT is None else LIMIT}")
        print(f"Sections to process: {len(sections_to_process)}")
        ...
        for i, payload in enumerate(sections_to_process):
            title_en = payload.get('name', {}).get('en', 'N/A')
            code = payload.get('code', 'N/A')
            
            print(f"\n[{i + 1}/{len(sections_to_process)}] {title_en} ({code})")
            print(f"Endpoint: POST https://{HOST}/api/content/shubhdarshan/v1/admin/darshan-sections")

            if DRY_RUN:
                print("[DRY RUN] Payload to be sent:")
                print(json.dumps(payload, indent=2, ensure_ascii=False))

            result = create_section_request(conn, payload)
            ...
            if not DRY_RUN and i < len(sections_to_process) - 1:
                time.sleep(REQUEST_DELAY_SECONDS)

        conn.close()

        print('\n--- SUMMARY ---')
        print(f"Total selected: {len(sections_to_process)}")
