...
const VIDEO_THUMBNAILS_FILE = path.join(__dirname, 'VideoThumbnailsThreeDTheatre_v2.json');
 ...
 * - Trim, lowercase
 * - Remove prefixes: Shri, Shree, Sri
 * - Collapse spaces, replace & with and, remove punctuation
 */
...
/**
 * Find matching card by exact or fuzzy title match.
 */
...
/**
 * Find matching episode by exact or fuzzy title match.
 */
...
/**
 * Group VideoThumbnailsThreeDTheatre entries by series.
 * Returns: [{ cardData: E0, episodes: [E1, E2, ...] }, ...]
 */
function groupBySeries(vtData) {
    const seriesMap = new Map(); // seriesIndex -> { cardData, episodes: [] }

    const keys = Object.keys(vtData).sort((a, b) => {
        const ma = a.match(/3DT_S(\d+)_E(\d+)/);
        const mb = b.match(/3DT_S(\d+)_E(\d+)/);
        if (!ma || !mb) return 0;
    ...
    for (const key of keys) {
        const m = key.match(/3DT_S(\d+)_E(\d+)/);
        if (!m) continue;

        const seriesIndex = parseInt(m[1]);
        const episodeIndex = parseInt(m[2]);
        const entry = vtData[key];

        if (!seriesMap.has(seriesIndex)) {
            seriesMap.set(seriesIndex, { cardData: null, episodes: [] });
        }

        const s = seriesMap.get(seriesIndex);

        if (episodeIndex === 0) {
            s.cardData = entry;
        } else {
            s.episodes.push(entry);
        }
    }

    return Array.from(seriesMap.keys()).sort((a, b) => a - b).map(i => seriesMap.get(i));
}

/**
 * Build streamingOptions from episode data.
 * When low/medium empty, derive from highQualityVideoUrl (master.m3u8).
 */
function buildStreamingOptions(epData) {
    const baseUrl = epData.highQualityVideoUrl || epData.AdaptiveStreamingVideoURL;
    const awsKey = epData.highQualityVideoKeyAWSObjectId || epData.AdaptiveStreamVideoKeyAWSObjectId || '';
    ...
    if (epData.AdaptiveStreamingVideoURL) {
        options.push({
            contentUrl: epData.AdaptiveStreamingVideoURL,
            streamingQuality: 'ADAPTIVE',
            awsKey: epData.AdaptiveStreamVideoKeyAWSObjectId || awsKey,
            sizeInKb: null
        });
    ...
    const qualities = [
        { quality: 'P_240', path: '240p/playlist.m3u8', url: epData.lowQualityVideoUrl },
        { quality: 'P_360', path: '360p/playlist.m3u8', url: epData.mediumQualityVideoUrl },
        { quality: 'P_540', path: '540p/playlist.m3u8', url: null },
        { quality: 'P_720', path: '720p/playlist.m3u8', url: epData.highQualityVideoUrl }
    ];
 ...
 */
function cardThumbnailDiffers(card, cardData) {
    const t = card.thumbnail || {};
    return (
        (t.imageUrl || '') !== (cardData.thumbnailUrl || cardData.thumbnailIconUrl || '') ||
        (t.title?.en || '') !== (cardData.videoTitle || '') ||
        (t.title?.hi || '') !== (cardData.hindiTitleText || '') ||
        (t.description?.en || '') !== (cardData.description || '') ||
        (t.description?.hi || '') !== (cardData.hindiDescriptionText || '')
    );
}
...
function episodeThumbnailDiffers(ep, epData) {
    ...
    return (
        (t.imageUrl || '') !== (epData.thumbnailUrl || epData.thumbnailIconUrl || '') ||
        (t.title?.en || '') !== (epData.videoTitle || '') ||
        (t.title?.hi || '') !== (epData.hindiTitleText || '') ||
        (t.description?.en || '') !== (epData.description || '') ||
        (t.description?.hi || '') !== (epData.hindiDescriptionText || '')
    );
}
...
/**
 * Get which card thumbnail fields changed. Returns { imageUrl, titleEn, titleHi, descEn, descHi }.
 */
function getCardThumbnailChanges(card, cardData) {
    const t = card.thumbnail || {};
    const img = cardData.thumbnailUrl || cardData.thumbnailIconUrl || '';
    const en = cardData.videoTitle || '';
    const hi = cardData.hindiTitleText || '';
    const descEn = cardData.description || '';
    const descHi = cardData.hindiDescriptionText || '';
    return {
...
/**
 * Get which episode thumbnail fields changed.
 */
function getEpisodeThumbnailChanges(ep, epData) {
    const t = ep.thumbnail || {};
    const img = epData.thumbnailUrl || epData.thumbnailIconUrl || '';
    const en = epData.videoTitle || '';
    const hi = epData.hindiTitleText || '';
    const descEn = epData.description || '';
    const descHi = epData.hindiDescriptionText || '';
    return {
        imageUrl: (t.imageUrl || '') !== img,
...
/**
 * Get which streaming qualities changed. Returns array of quality names.
 */
...
async function main() {
    try {
        ...
        const seriesList = groupBySeries(vtData);
        const limitedSeries = LIMIT ? seriesList.slice(0, LIMIT) : seriesList;

        console.log(`Source: ${seriesList.length} series, ${seriesList.reduce((s, x) => s + x.episodes.length, 0)} episodes`);
        console.log(`Response: ${responseCards.length} cards`);
        console.log(`Processing: ${limitedSeries.length} series`);
        console.log('');

        for (let sIdx = 0; sIdx < limitedSeries.length; sIdx++) {
            const { cardData, episodes } = limitedSeries[sIdx];
            if (!cardData) {
                console.log(`[Series ${sIdx}] Skipping - no E0 card data`);
                continue;
            }

            const cardTitle = cardData.videoTitle;
            ...
            if (!existingCard) {
                ...
                const payload = {
                    thumbnail: {
                        title: { hi: cardData.hindiTitleText || '', en: cardData.videoTitle || '' },
                        imageUrl: cardData.thumbnailUrl || cardData.thumbnailIconUrl || '',
                        description: { hi: cardData.hindiDescriptionText || '', en: cardData.description || '' },
                ...
                if (cardThumbnailDiffers(existingCard, cardData)) {
                    const changes = getCardThumbnailChanges(existingCard, cardData);
                    const changeParts = [];
            ...
            for (let eIdx = 0; eIdx < episodes.length; eIdx++) {
                ...
                const epTitle = epData.videoTitle;
                ...
                if (!existingEp) {
                    ...
                    const payload = {
                        thumbnail: {
                            title: { hi: epData.hindiTitleText || '', en: epData.videoTitle || '' },
                            description: { hi: epData.hindiDescriptionText || '', en: epData.description || '' },
                            imageUrl: epData.thumbnailUrl || epData.thumbnailIconUrl || '',
                            displayOrientation: 'LANDSCAPE'
                        },
                        streamingOptions: streamingOpts,
                        episodeNumber: (eIdx + 1).toString(),
                        locked: epData.isFreeShow === 'true' ? false : true,
                        status: 'PUBLISHED',
                        contentType: 'FLAT_3D',
                        sequenceNo: eIdx
                    };
